...
%如果没有设置L的初始值，则设L为单位矩阵
if(exist('L')~=1)
 fprintf(['Initial starting point not specified.\nStarting with identity matrix.\n']);  
 L=eye(size(x,1));%eye() 生成一个单位矩阵,  size()矩阵某一个维度的大小
end;
tic

% checks，验证初始的矩阵L的长度和输入X的每一个向量x长度是否一致
D=size(L,2);
x=x(1:D,:);
...
%i=randperm(size(x,2));
i=1:size(x,2);
tr=ceil(size(x,2)*(1-pars.validation));%ceil() 大于的最小整数，pars.validation为训练集的比例
ve=size(x,2)-tr;
xo=x;
yo=y;
...
xv=xo(:,i(tr+1:end));
yv=yo(i(tr+1:end));
%至此，x和y是训练集，xv和yv是验证集

verify=[];besterr=inf;%inf 表示正无穷大
clear('xo','yo');
lowesterr=inf;
verify=[];
bestL=L;
%上述变量应该是用于存储训练的结果的，包括最小的误差，矩阵L和verify

if(~pars.quiet)%表示是否输出
pars
end;
...
%找最近的邻居,NN中存放的是最近邻的索引，gen存放的是数据对，由最近邻形成的数据对
[gen,NN]=getGenLS(x,y,Kg,pars);
obj=zeros(1,pars.maxiter);%zeros生成0矩阵
